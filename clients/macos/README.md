...
設定頁「輸入行為」群組的「移除句尾句點」（`stripTrailingPunctuation`，預設關閉）是目前唯一
內建的清理規則，行為邊界如下：

- 只處理句子**最後**一個非空白字元；句中任何標點（包含句中的句點）都不會被動到。
- 只認「句點」這一類字元：中文的「。」（U+3002）、全形拉丁句點「．」（U+FF0E）與半形
  「.」（U+002E）。問號「？」與驚嘆號「！」不在範圍內——使用者的原始需求只針對句點，
  這兩者承載語氣，移除會遺失資訊，因此刻意分開處理、不一併移除。
...
- 這條規則的開關狀態是在每次 `TranscriptProcessor.process` 呼叫時即時讀取，不是只在啟動時
